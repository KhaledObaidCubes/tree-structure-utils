<template>
  <div class="container">
    <h3 class="caption">
      ...
      <button @click="nodes?.addNodeToTarget(nodes, new TreeManager({ name: 'a9', idooo: '', checked: false, children: [] }))">ADD</button>
      <button v-if="!!nodes?.depth || !!nodes?.indexInParent" @click="nodes?.removeNode(nodes)">DELETE</button>
...
const child = defineAsyncComponent(() => import('../components/generic-tree.vue'))
</script>
<style scoped>
.container {
